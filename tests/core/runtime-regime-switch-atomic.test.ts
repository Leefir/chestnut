...
describe('Runtime regime switch atomicity (phase 600 / A.regime-switch-atomicity)', () => {
  ...
  it('step 6 newSessionManager.save throws → oldSessionManager 不变（reorder 真生效）', async () => {
    ...
    const oldSessionManager = (runtime as unknown as RuntimeTestInternals).sessionManager;
    ...
    expect((runtime as unknown as RuntimeTestInternals).sessionManager).toBe(oldSessionManager);
    // lastIdentityHash should NOT be updated (D7 自愈)
    expect((runtime as unknown as RuntimeTestInternals).lastIdentityHash).toBe('identity-A');
  });
  ...
  it('happy path：save 成功 → commit this.sessionManager + audit REGIME_SWITCH', async () => {
    ...
    expect((runtime as unknown as RuntimeTestInternals).sessionManager).toBe(capturedNewSessionManager);
    // lastIdentityHash should be updated
    expect((runtime as unknown as RuntimeTestInternals).lastIdentityHash).toBe('identity-B');
