/**
 * StepExecutor tool content truncate helper (phase 140 §5.D Step D).
 *
 * Atomic invariant: truncate + emit content_size are inseparable.
 *
 * Owner: step-executor module.
 */

import { createHash } from 'node:crypto';

export const STEP_EXECUTOR_TOOL_MAX_CHARS = 200;
...
export function truncateToolContent(
  ...
  const maxChars = options?.maxChars ?? STEP_EXECUTOR_TOOL_MAX_CHARS;
  if (maxChars < 1) {
