...
describe('phase 1487: contract-events composer', () => {
  ...
  it('single problem (1 pair) → trace cmd with real ids + shadow recommendation', () => {
    ...
    expect(result!.text).toContain('chestnut claw worker-1 trace --contract 1780-abcd');
  ...
  it('multi problem (2 pairs) → enumerate trace cmds + plural intro', () => {
    ...
    expect(result).not.toBeNull();
    expect(result!.text).toContain('chestnut claw worker-1 trace --contract 1780-abcd');
    expect(result!.text).toContain('chestnut claw worker-2 trace --contract 1780-cdef');
  ...
  it('trims whitespace around pairs', () => {
    ...
    expect(result).not.toBeNull();
    expect(result!.text).toContain('chestnut claw worker-1 trace --contract abc');
    expect(result!.text).toContain('chestnut claw worker-2 trace --contract def');
  });
});
