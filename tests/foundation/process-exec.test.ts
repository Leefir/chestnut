/**
 * ProcessExec execFile tests
 *
 * Covers the new execFile(command, args, options) entry point:
...
import { exec, execFile } from '../../src/foundation/process-exec/index.js';
import { ProcessExecError } from '../../src/foundation/process-exec/types.js';

describe('ProcessExec execFile', () => {
  const workDir = tmpdir();

  // ── basic execution ─────────────────────────────────────────────────────

  it('should execute command with args', async () => {
    const result = await execFile('echo', ['hello', 'world'], { cwd: workDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout.trim()).toBe('hello world');
  });

  it('should return empty stderr on success', async () => {
    const result = await execFile('echo', ['ok'], { cwd: workDir });
  ...
  it('should pass args with special chars verbatim', async () => {
    const result = await execFile('echo', ['$', 'HOME', '|', 'grep'], { cwd: workDir });
  ...
  it('should pass args with single quotes verbatim', async () => {
    const result = await execFile('echo', ["it's a test"], { cwd: workDir });
    expect(result.stdout.trim()).toBe("it's a test");
  });

  // ── contrast with exec (shell) ──────────────────────────────────────────

  it('execFile does not expand $VAR, exec does', async () => {
    const direct = await execFile('echo', ['$HOME'], { cwd: workDir });
    expect(direct.stdout.trim()).toBe('$HOME');

    const shell = await exec('echo $HOME', { cwd: workDir });
  ...
  it('should throw ProcessExecError on non-existent command', async () => {
    await expect(
      execFile('nonexistent_command_xyz_12345', [], { cwd: workDir }),
    ).rejects.toThrow(ProcessExecError);
  });

  it('should throw ProcessExecError on non-zero exit code', async () => {
    try {
      await execFile('node', ['-e', 'process.exit(42)'], { cwd: workDir });
  ...
  it('should throw ProcessExecError on timeout', async () => {
    await expect(
      execFile('node', ['-e', 'setTimeout(() => {}, 60000)'], {
        cwd: workDir,
        timeout: 1000,
      }),
    ).rejects.toThrow(ProcessExecError);
    ...
    try {
      await execFile('node', ['-e', 'setTimeout(() => {}, 60000)'], {
        cwd: workDir,
        timeout: 1000,
      });
      expect.fail('should have thrown');
  ...
  it('should throw ProcessExecError when signal is already aborted', async () => {
    ...
    await expect(
      execFile('echo', ['should-not-run'], { cwd: workDir, signal: controller.signal }),
    ).rejects.toThrow();
  });
  ...
  it('should clamp timeout to MIN (1000ms)', async () => {
    ...
    const result = await execFile('node', ['-e', 'setTimeout(() => {}, 100)'], {
      cwd: workDir,
  ...
  it('should include node bin dir in PATH', async () => {
    const result = await execFile('node', ['-e', 'console.log(process.env.PATH)'], {
      cwd: workDir,
    });
