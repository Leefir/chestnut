...
describe('transport nullness single source-of-truth (phase 877 / r113 E fork)', () => {
  ...
  describe('online mode: stop sequence broadcast', () => {
    it('dropConnection during stop broadcasts connection_dropped before transport.close', async () => {
      ...
      const broadcastCalls = transport.broadcast.mock.calls;
      const connDroppedCall = broadcastCalls.find(([data]) => {
        const msg = JSON.parse(data as string);
        return msg.type === 'connection_dropped' && msg.connectionId === 'c1';
      });
      expect(connDroppedCall).toBeDefined();
      expect(transport.close).toHaveBeenCalledOnce();

      const dropOrder = broadcastCalls.findIndex(([data]) => JSON.parse(data as string).type === 'connection_dropped');
      const closeOrder = transport.close.mock.invocationCallOrder[0];
      const broadcastOrder = transport.broadcast.mock.invocationCallOrder[dropOrder];
